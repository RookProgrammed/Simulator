...
Cache_wt::Cache_wt(uint32_t size, uint32_t associativity, uint32_t blkSize,
		enum ReplacementPolicy replType, uint32_t delay, bool is_bottom):
		Cache(size, associativity, blkSize, replType, delay, is_bottom),cSize(size),
...
uint32_t Cache_wt::getAssociativity() {
	return associativity;
}

uint32_t Cache_wt::getNumSets() {
	return numSets;
}

uint32_t Cache_wt::getBlockSize() {
	return blkSize;
}

int Cache_wt::getWay(uint32_t addr) {
...
bool Cache_wt::policy() {
	return false;
}

// Invalidates a cache block given its address
void Cache_wt::invalidate(uint32_t addr) {
	uint32_t _addr = addr / blkSize;
	uint32_t setIndex = (numSets - 1) & _addr;
	uint32_t addrTag = _addr / numSets;
	for (int i = 0; i < (int) associativity; i++) {
...
bool Cache_wt::sendReq(Packet * pkt){
...
void Cache_wt::recvResp(Packet* readRespPkt){
	DPRINTF(DEBUG_CACHE,
				...
				if (pkt->isReq) {
					...
					if (!is_bottom) {
						if (pkt->type == PacketTypeLoad) {
							...
							((Cache_wt*) prev)->invalidate(b->getTag());
						} else {
							// Invalidate L1D block for fetch
							((Cache_wt*) prev2)->invalidate(b->getTag());
						}
					}						
...
/*You should complete this function*/
void Cache_wt::dumpRead(uint32_t addr, uint32_t size, uint8_t* data){
