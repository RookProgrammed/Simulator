...
int MSHR::mshr_lookup(Packet* pkt){
	int i = 0;
	for(i; i<8; i++){
		if(req_addr[i][0] == pkt->addr && valid[i][0] == 1){
			return i;
		}
	}
	return -1;
...
void MSHR::mshr_insert(Packet* pkt){
	int n = get_empty();
	valid[n][0] = 1;
	req_addr[n][0] = pkt->addr;
}
void MSHR::mshr_subinsert(Packet* pkt, uint32_t addr){
	for(int n=0;n<8;n++){
		for(int m=0;m<2;m++){
			if(mshr_subentryisfull(addr) == false && req_addr[n][m] == addr){
...
bool MSHR::mshr_subentryisfull(uint32_t addr){
	bool isfull = false;
	int count = 0;
	for(int n =0; n<8; n++){
		for(int m=0;m<2;m++){
			if(req_addr[n][m] == addr){
					if(m == 1){
						return isfull = true;
					}	
			}
...
void MSHR::mshr_delete(Packet* pkt){
	for(int n=0;n<8;n++){
		for(int m=0;m<2;m++){
			if(req_addr[n][m] == pkt->addr){
				valid[n][m] = 0;
			}
		}
	}
		...
		if (reqQueue.front()->ready_time <= currCycle) {
			...
			if (!respPkt->isReq){
				...
				if (way == -1){
					...
					write_back_buffer.push(wb_block);
					if (next->sendReq(respPkt)){
						reqQueue.pop();
						mshr_L1->mshr_delete(respPkt);
					}
					else break;
				}
				else{
				...
				if (respPkt->isWrite) {
					...
					if (way == -1 || !blocks[setIndex][way]->getValid()){
						...
						if(mshr_L1->mshr_lookup(respPkt) != -1){ //if find one same request in MSHR
								if(mshr_L1->mshr_subentryisfull(respPkt->addr) == false){ //if there is an empty subentry
									mshr_L1->mshr_subinsert(respPkt, respPkt->addr);
									printf("MSHR hit, add a subentry\n");
								}
								else if(mshr_L1->mshr_isfull() == false){ // else if there is still an empty entry
									mshr_L1->mshr_insert(respPkt);
									printf("MSHR hit, add a new entry\n");
								}
								// else stall
								else break;
						}
						else{  // if not find a same request in MSHR
							if(mshr_L1->mshr_isfull() == false){  //if there is an empty entry
								mshr_L1->mshr_insert(respPkt);
								printf("MSHR miss, add an entry to MSHR\n");
							}
							// else stall;
							else break;
						}
