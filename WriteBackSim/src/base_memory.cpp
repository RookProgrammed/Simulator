		...
		if (reqQueue.front()->ready_time <= currCycle) {
			...
			if (!respPkt->isReq){
				...
				if (respPkt->isWrite) {
					...
					mem_region_t* mem_block = getMemRegion(block_addr, blkSize);
					uint8_t * temp_mem = new uint8_t[blkSize];
					for (uint32_t i=0; i< blkSize; i++){
						temp_mem[i] = mem_block->mem[i];
					}
					int index = respPkt->addr - block_addr ;
					for (uint32_t i=0; i < respPkt->size;i++){
						temp_mem[i+index] = *(respPkt->data +i);
					}
					//perform the read
					delete respPkt->data;
					respPkt->data = new uint8_t[blkSize];
					for (uint32_t i = 0; i < blkSize; i++) {
						*(respPkt->data + i) = temp_mem[i];
					}
